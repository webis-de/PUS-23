...
class Arno_Section:
    ...
    def get_fullheading(self):
        def recursive_appending(self, heading):
            if not self.parent:
                return
            heading.append(self.parent.heading)
            recursive_appending(self.parent, heading)
        heading = [self.heading]
        recursive_appending(self, heading)
        return '/'.join(heading[::-1])
