...
class Revision:
    ...
    # Ab hier Arno stuff...
    def get_arno_sections(self, headline_range = range(1,7)): # Arno
        """
        ...
        indices = list(finditer(r'|'.join(['<h' + str(i) + '.*?h' + str(i) + '>' for i in range(1,7)]), self.html))
        if not indices:
            return []
    ...
    def get_specific_sections(self, selection): # Arno
        """
    ...
    def get_section_headings(self, show_parents=True): # Arno
